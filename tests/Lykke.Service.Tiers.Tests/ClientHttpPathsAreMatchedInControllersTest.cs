...
namespace Lykke.Service.Tiers.Tests
{
    public class ClientHttpPathsAreMatchedInControllersTest
    {
        ...
        {
            var clientInterface = typeof(ITiersClient);
            ...
            {
                var implementingController =
                    controllers.FirstOrDefault(c => c.GetInterfaces().Any(i => i == apiInterface));
                ...
                {
                    var refitAttr =
                        apiMethod.CustomAttributes.FirstOrDefault(a => _refitAttrs.Any(i => i == a.AttributeType));
                    if (refitAttr == null)
                    {
                    ...
                    var controllerRouteAttr =
                        implementingController.CustomAttributes.FirstOrDefault(a => a.AttributeType == _routeAttrType);
